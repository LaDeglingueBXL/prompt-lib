...
import { openGenerator } from "./generator.js";
import { runImportIfQueued } from "./importer.js";
...
async function init() {
  ...
  registerSW();
  await db.ready;
  await runImportIfQueued(db);
  await refreshCount();
  initSearch();

  newBtn.addEventListener("click", () => openModal(emptyEntry(), { isNew: true }));
  document.getElementById("open-gen").addEventListener("click", openGenerator);
  document.getElementById("btn-deploy").addEventListener("click", exportData);
