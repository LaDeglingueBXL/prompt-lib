...
import { esc, escRe, copyText, flashCopied } from "./util.js";
...
export function initSearch() {
  ...
  // refresh list when entries are added/edited/deleted/duplicated
  document.addEventListener("entries:changed", () => run(searchEl.value));

  // delegate clicks: copy button OR open modal on card body
  resultsEl.addEventListener("click", (e) => {
    const copyBtn = e.target.closest("[data-copy]");
    if (copyBtn) {
      copyEntry(copyBtn.getAttribute("data-copy"));
      return;
    }
    const card = e.target.closest(".card");
    if (card) {
      const entry = current.find((x) => x.id === card.dataset.id);
